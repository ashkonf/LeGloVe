...
from example import find_nearest_neighbors, main
...
class TestFindNearestNeighbors:
    ...
    def test_find_nearest_neighbors_distances(self, mock_glove_class: Mock) -> None:
        ...
        with patch('example.pprint') as mock_pprint:
            ...
            result_dict = {word: distance for word, distance in results}
...
class TestMain:
    ...
    @patch('example.parse_arguments')
    @patch('example.train_and_save_model')
    @patch('example.find_nearest_neighbors')
    def test_main_train_mode(
        self, 
        mock_find_neighbors: Mock, 
        mock_train: Mock, 
    ...
    def test_main_no_input_error(self, mock_parse_args: Mock) -> None:
        ...
        with pytest.raises(ValueError, match="Must provide either a training directory or a model file to load"):
            main()
