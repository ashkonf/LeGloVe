...
from train import tokenize_text, read_corpus, train_and_save_model
...
class TestReadCorpus:
    ...
    @patch('train.extract_text')
    def test_read_corpus_empty_text(self, mock_extract_text: Mock, temp_dir: str) -> None:
...
class TestTrainAndSaveModel:
    ...
    def test_train_and_save_model_basic(
        ...
        train_and_save_model(temp_dir, model_name="TestModel", num_epochs=5, parallel_threads=2)
        ...
        mock_glove.fit.assert_called_once_with("mock_matrix", epochs=5, no_threads=2, verbose=True)
    ...
    def test_train_and_save_model_default_params(
        ...
        mock_glove.fit.assert_called_once_with("mock_matrix", epochs=10, no_threads=1, verbose=True)
